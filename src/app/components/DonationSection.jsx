      ...
      <section className="sec-dark">
        <div className="wrap">
          <div className="reveal" style={{ textAlign:'center', marginBottom:'3.5rem' }}>
            <span className="section-badge light">Seva Opportunities</span>
            <h2 style={{ fontSize:'clamp(1.8rem,3.5vw,2.75rem)', fontWeight:700, color:'white', marginBottom:'.75rem' }}>
              Serve the <span className="gradient-text-gold">Divine</span>
            </h2>
            <div className="gold-line-center" />
            <p style={{ color:'rgba(255,255,255,.6)', maxWidth:'540px', margin:'0 auto', fontSize:'.95rem', lineHeight:1.75 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))', gap:'1.5rem' }}>
            {sevas.map((seva, i) => (
              <div key={seva.title} className={`card-glass reveal d${(i+1)*100}`} style={{ padding:'2rem' }}>
                ...
                <h3 style={{ fontSize:'1.1rem', fontWeight:700, color:'white', marginBottom:'.75rem', fontFamily:'var(--font-cinzel),serif' }}>{seva.title}</h3>
                <p style={{ fontSize:'.875rem', lineHeight:1.75, color:'rgba(255,255,255,.6)', marginBottom:'1.5rem' }}>{seva.desc}</p>
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'1rem 1.25rem', background:'rgba(237,104,0,.08)', borderRadius:'.875rem', marginBottom:'1.25rem', border:'1px solid rgba(237,104,0,.15)' }}>
                  <span style={{ fontSize:'.75rem', fontWeight:600, color:'rgba(255,255,255,.5)', textTransform:'uppercase', letterSpacing:'.1em' }}>Suggested Seva</span>
