            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))', gap:'1rem' }}>
              ...
              <div>
                ...
                <input style={fInput(errors.pan)} type="text" maxLength={10} placeholder="ABCDE1234F"
                  value={form.pan} onChange={e => { const v = e.target.value.toUpperCase(); setForm(f => ({ ...f, pan: v })); setErrors(er => ({ ...er, pan: '' })); }} onBlur={() => { const v = form.pan.toUpperCase(); setForm(f => ({ ...f, pan: v })); setErrors(er => ({ ...er, pan: rules.pan(v) })); }} />
