...
export default function EventRegisterButton({ eventName, eventId }) {
  ...
  const openModal = () => { setSubmitted(false); setForm(f => ({ ...f, event_id: eventId || '', event_name: eventName || '' })); setModal(true); };
  ...
  // When event selection changes, update event_id and event_name together
          ...
          <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
            <div>
              ...
              <select required style={fInput} value={form.event_id} onChange={handleEventChange}>
                ...
                {events.length > 0
                  ? events.map(ev => <option key={ev.id} value={ev.id}>{ev.name}{ev.date ? ` — ${ev.date} ${ev.month || ''}` : ev.month ? ` — ${ev.month}` : ''}</option>)
